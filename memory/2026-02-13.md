...
## Bob App Benchmark (successful — v3)
- NUM_PARALLEL=3, Bob warm, 120B loaded
- **Wall time: 6m9s** — all 7 files written ✅
- Tests: 5/5 passed, app runs correctly (python main.py multiply 4 5 → "4 * 5 = 20")
- Used 131K input tokens, 4.6K output tokens
- Files: calc.py, validator.py, formatter.py, history.py, test_calc.py, main.py, README.md

## Final Benchmark Comparison
| | 8 Minis (parallel) | Bob (solo) |
|---|---|---|
| Wall time | ~130s | 369s |
| Files written | 5/8 ❌ | 7/7 ✅ |
| Tests pass | N/A | 5/5 ✅ |
| App runs | ❌ | ✅ |

**Conclusion:** Bob slower but reliable. Minis fast but unreliable at tool use (file writing). Mini swarm → Bob review pipeline is the right architecture.

## Ollama Stability & Parallel Config
- Bob's transcript showed repeated "500 do load request: Post http://127.0.0.1:XXXXX/load: EOF" errors
- Root cause: OLLAMA_NUM_PARALLEL=12 reserves too many KV cache slots → 120B model (65GB) can't load → runner crashes
- Fix: dialed back to OLLAMA_NUM_PARALLEL=3 — safe for both 120B and 20B
- Bob warm-up with NUM_PARALLEL=3: 88s (loading 65GB model into VRAM)
- Key lesson: high parallel slots work for 20B but crash the 120B. Need different configs per model or dynamic switching.
- Retrying Bob app benchmark at NUM_PARALLEL=3 (in progress)
